...
export function useReadings() {
  ...
  }, [customerName, mobile, weight, date, sampleType, selectedReadingIds, hasComposition, displayPrim, primKey, elementSum, elementValues])
  ...
  const saveSampleToDb = useCallback(async () => {
    ...
    try {
      ...
      if (Object.keys(finalPayload).length > 0) {
        ...
        const deltaMeta = {
          primaryElement: primKey,
          primaryDelta: machineDeltaRows.find(row => row.sym === primKey)?.delta ?? null,
  ...
  }, [validateForm, readings, selectedReadingIds, customerName, sampleCat, sampleType, weight, mobile, date, primKey, displayPrim, elementValues, editingSampleId, machineBaseline, setFieldError, mapServerErrors])
  ...
  const handlePrint = useCallback(async () => {
    ...
    try {
      ...
      const srNoDisplay = saved.srNo || saved.jobRef || saved.sampleId
      toast.success(`Printed Sr No ${srNoDisplay}${body?.printer ? ` (${body.printer})` : ''}`, {
        duration: 3000,
        position: 'top-right',
      })
  ...
  }, [canPrint, printing, saving, saveSampleToDb, entryMode, prepareNextItem, prepareFreshEntry, setFieldError, mapServerErrors])
