...
const STATUS_LABELS = {
  pending_review:   { label: 'Not Yet Printed', cls: 'bg-yellow-100 text-yellow-700' },
  expert_review:    { label: 'Not Yet Printed', cls: 'bg-yellow-100 text-yellow-700' },
...
export default function SamplesPage() {
  ...
  const handleModify = async (sampleId) => {
    ...
    try {
      ...
      console.error(err)
      alert('Could not load sample for editing. Please try again.')
    } finally {
  ...
  const handleRevise = async (sampleId) => {
    setLoadingSample(sampleId)
    try {
      const reviseRes = await fetch(`/api/samples/${sampleId}/revise`, { method: 'POST' })
      const reviseBody = await reviseRes.json().catch(() => ({}))
      if (!reviseRes.ok) throw new Error(reviseBody?.detail || reviseBody?.error || 'Could not revise sample')
      ...
      console.error(err)
      alert(err.message || 'Could not revise sample. Please try again.')
    } finally {
      setLoadingSample(null)
    }
  }
  ...
  const handlePrint = async (sampleId, jobRef) => {
    ...
    try {
      const res = await fetch(`/api/samples/${sampleId}/report`, { method: 'POST' })
      const body = await res.json().catch(() => ({}))
      if (!res.ok) throw new Error(body?.detail || body?.error || 'Print failed')
      alert(`Print job sent${body?.printer ? ` to ${body.printer}` : ''} for sample ${jobRef}.`)
      fetchSamples()
    } catch (err) {
      console.error(err)
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-700">
              {samples.map((s, idx) => {
                ...
                const { sampleCat, sampleType, weight, srNo, mobile } = s.parsedItemDesc || {}
                ...
                const modifiedDate = s.updated_at
                  ? new Date(s.updated_at).toLocaleString('en-GB', { day: '2-digit', month: '2-digit', year: '2-digit', hour: '2-digit', minute: '2-digit' })
                  : '—'
                ...
                return (
                  <tr key={s.id} className={idx % 2 === 0 ? 'bg-slate-800' : 'bg-slate-750'}>
                    <td className="px-3 py-2.5 font-mono font-bold text-amber-400">{srNo || s.job_ref}</td>
                    <td className="px-3 py-2.5 text-slate-200">{s.customer_name || '—'}</td>
                    <td className="px-3 py-2.5 text-slate-300">{sampleCat || '—'}</td>
                    <td className="px-3 py-2.5 text-slate-300 text-xs">{sampleType || '—'}</td>
                    <td className="px-3 py-2.5 text-slate-400 font-mono text-xs">{mobile || '—'}</td>
                    <td className="px-3 py-2.5 text-center text-slate-300">{weight || '—'}</td>
                        ...
                        {readings.length > 0 && (
                          <div className="flex flex-wrap justify-center gap-0.5">
                            {readings.map(r => (
                              <span
                                key={r.id}
                                title={r.excluded ? `#${r.nbr || r.num} (excluded)` : `#${r.nbr || r.num}`}
                          ...
                          {sorted.map(el => (
                            <span
                              ...
                              className={`text-xs font-mono whitespace-nowrap ${
                                el.element === 'Au' ? 'text-amber-400 font-bold' :
                                el.element === 'Ag' ? 'text-slate-300' :
                                'text-slate-400'
                              }`}
                            >
                    ...
                    <td className="px-3 py-2.5 text-center text-xs text-slate-400">{createdDate}</td>
                    <td className="px-3 py-2.5 text-center text-xs text-slate-400">{modifiedDate}</td>
                    <td className="px-3 py-2.5 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        >
                          {printingSample === s.id ? '⏳ Printing…' : '🖨 Print'}
                        </button>
                        <button
                          onClick={() => st.label === 'Printed' ? handleRevise(s.id) : handleModify(s.id)}
                          disabled={loadingSample === s.id}
                          title={st.label === 'Printed' ? 'Re-open printed sample for revision' : 'Load for editing on Dashboard'}
                        ...
                        >
                          {loadingSample === s.id ? '⏳ ...' : st.label === 'Printed' ? '↺ Revise' : '✎ Modify'}
                        </button>
                      </div>
                    </td>
                  </tr>
